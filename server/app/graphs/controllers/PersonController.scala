...
class PersonController @Inject()(repo: PersonRepository,
  ...
  def index: Action[AnyContent] = Action { implicit request =>
    Ok(views.html.pages.addpeople(personForm))
  }
  ...
  def addPerson(): Action[AnyContent] = Action.async { implicit request =>
    ...
    personForm.bindFromRequest().fold(
      ...
      errorForm => {
        Future.successful(Ok(views.html.pages.addpeople(errorForm)))
      },
      // There were no errors in the from, so create the person.
      person => {
        repo.create(person.name, person.age).map { _ =>
          // If successful, we simply redirect to the index page.
          Redirect(graphs.controllers.routes.PersonController.index).flashing("success" -> "user.created")
        }
      }
    )
  }
  ...
  def viewPersons: Action[AnyContent] = Action.async { implicit request =>
    repo.list().map { people =>
      Ok(views.html.pages.people(people))
    }
  }
